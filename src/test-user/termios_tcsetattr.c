...
int main(void) {
  ...
  TEST_ASSERT(tcgetattr(0, &saved_tio) == 0);

  for (i = 0; i < 3; ++i) {
    action = optional_actions[i];
    memset(&tio, 0, sizeof(struct termios));

    fd = open("/a.txt", O_CREAT|O_WRONLY|O_TRUNC, 0644);
    TEST_ASSERT(tcsetattr(fd, action, &tio) == -1);
    TEST_ASSERT(errno == ENOTTY);

    TEST_ASSERT(tcsetattr(100, action, &tio) == -1);
    TEST_ASSERT(errno == EBADF);

    TEST_ASSERT(tcsetattr(0, action, &tio) == 0);
    TEST_ASSERT(tcgetattr(0, &tio) == 0);
    TEST_ASSERT(tio.c_oflag == 0);
    TEST_ASSERT(tio.c_iflag == 0);
    TEST_ASSERT(tio.c_cflag == 0);
    TEST_ASSERT(tio.c_lflag == 0);
    TEST_ASSERT(tio.c_cc[VTIME] == 0);
    TEST_ASSERT(tio.c_cc[VMIN] == 0);

    TEST_ASSERT(tcsetattr(0, action, &saved_tio) == 0);
    TEST_ASSERT(tcgetattr(0, &tio) == 0);
